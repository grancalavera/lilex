...
module.exports = function parseLexicon(lines) {
...
function parseTerm(lines) {
  ...
  if (!!term.pattern) term.pattern = from2Dto1D(term.pattern)

  return term
}

function from2Dto1D(pattern) {
  return pattern.reduce((pattern1D, row, y) => {
    return pattern1D.concat(row.reduce((innerPattern1D, isAlive, x) => {
      if (!isAlive) return innerPattern1D
      return innerPattern1D.concat({x, y})
    }, []))
  }, [])
}
